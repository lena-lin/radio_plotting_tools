...
def get_gaussian_components(aips_cc_data):
    ...
    y_positions = (aips_cc_data.data['DELTAY'] * u.degree).to(u.mas)
    major_axes = (aips_cc_data.data['MAJOR AX'] * u.degree).to(u.mas)
    minor_axes = (aips_cc_data.data['MINOR AX'] * u.degree).to(u.mas)
    phi_ellipse = (aips_cc_data.data['POSANGLE'])

    # Shift Core to (0,0)
    x_positions = x_positions - x_positions[0] # in case first component fits core!!
    x_positions = y_positions - y_positions[0]
